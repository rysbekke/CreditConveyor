...
//namespace СreditСonveyor.Beeline
namespace СreditСonveyor.Webcam
{
    public partial class WebCam : System.Web.UI.Page
    {
        ...
        static string fileupl = ConfigurationManager.ConnectionStrings["fileupl"].ToString();
        ...
        protected void btnSavePhoto_Click(object sender, EventArgs e)
        {
            ...
            //string filedir = "Credits\\BeeCredits";
            //int? role = dbRWZ.Requests.Where(r => r.RequestID == int.Parse(reqid)).FirstOrDefault().AgentRoleID;

            //if ((role == 1) || (role == 4) || (role == 12)) filedir = "Credits\\Nurcredits";
            //if ((role == 8) || (role == 9) || (role == 18)) filedir = "Credits\\Beecredits";
            //if ((role == 13) || (role == 14) || (role == 22)) filedir = "Credits\\Partcredits";
            if (reqid == "n") 
            {
                //Nurtelecom.Nurtelecom.imageSrting = HiddenField1.Value;
