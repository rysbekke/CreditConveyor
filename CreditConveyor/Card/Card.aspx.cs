...
namespace СreditСonveyor.Card
{
    public partial class Card : System.Web.UI.Page
    {
        ...
        public void refreshGrid()
        {
            ...
            if (rle == 20) { lst = ItemController.GetRequestsAllForExpert(usrID, agentRoleID, nRequest, inn, surname, name, dt1, dt2, connectionStringR, chkbxlistStatus.Items[0].Selected, chkbxlistStatus.Items[1].Selected, chkbxlistStatus.Items[2].Selected, chkbxlistStatus.Items[3].Selected, chkbxlistStatus.Items[4].Selected, chkbxlistStatus.Items[5].Selected, chkbxlistStatus.Items[6].Selected, chkbxlistStatus.Items[7].Selected, chkbxlistStatus.Items[8].Selected, chkbxlistStatus.Items[9].Selected, chkbxlistStatus.Items[10].Selected, gvRequests.PageIndex, gvRequests.PageSize); } //Эсперты ГБ Капитал
            else if (rle == 19) { lst = ItemController.GetRequestsForExpert(usrID, agentRoleID, nRequest, inn, surname, name, dt1, dt2, connectionStringR, chkbxlistStatus.Items[0].Selected, chkbxlistStatus.Items[1].Selected, chkbxlistStatus.Items[2].Selected, chkbxlistStatus.Items[3].Selected, chkbxlistStatus.Items[4].Selected, chkbxlistStatus.Items[5].Selected, chkbxlistStatus.Items[6].Selected, chkbxlistStatus.Items[7].Selected, chkbxlistStatus.Items[8].Selected, chkbxlistStatus.Items[9].Selected, chkbxlistStatus.Items[10].Selected, gvRequests.PageIndex, gvRequests.PageSize); } //Эксперты филиала Капитал
            else if ((rle == 4) || (rle == 9)) { lst = ItemController.GetRequestsAllForAdmin(usrID, agentRoleID, nRequest, orgID, inn, surname, name, dt1, dt2, connectionStringR, chkbxlistStatus.Items[0].Selected, chkbxlistStatus.Items[1].Selected, chkbxlistStatus.Items[2].Selected, chkbxlistStatus.Items[3].Selected, chkbxlistStatus.Items[4].Selected, chkbxlistStatus.Items[5].Selected, chkbxlistStatus.Items[6].Selected, chkbxlistStatus.Items[7].Selected, chkbxlistStatus.Items[8].Selected, chkbxlistStatus.Items[9].Selected, chkbxlistStatus.Items[10].Selected, gvRequests.PageIndex, gvRequests.PageSize); } //Админы Билайн
            else { lst = ItemController.GetRequestsForAgent(usrID, agentRoleID, nRequest, inn, surname, name, dt1, dt2, connectionStringR, chkbxlistStatus.Items[0].Selected, chkbxlistStatus.Items[1].Selected, chkbxlistStatus.Items[2].Selected, chkbxlistStatus.Items[3].Selected, chkbxlistStatus.Items[4].Selected, chkbxlistStatus.Items[5].Selected, chkbxlistStatus.Items[6].Selected, chkbxlistStatus.Items[7].Selected, chkbxlistStatus.Items[8].Selected, chkbxlistStatus.Items[9].Selected, chkbxlistStatus.Items[10].Selected, gvRequests.PageIndex, gvRequests.PageSize); } //Агенты Нуртелеком
            if (lst.Count > 0)
            {
                var lst4 = (from r in lst
                            ...
                            select new { r.RequestID, r.CreditID, OfficeShortName = o.ShortName, r.Surname, r.CustomerName, r.Otchestvo, r.IdentificationNumber, r.CreditPurpose, r.ProductPrice, r.AmountDownPayment, r.RequestSumm, r.AgentUsername, r.RequestDate, r.RequestStatus, r.GroupID, r.BranchID, r.OrgID, r.StatusDate }).ToList();
                var lst5 = (from r in lst4
                            join b in dbRWZ.Branches on r.BranchID equals b.ID
                            select new { r.RequestID, r.CreditID, b.ShortName, r.OfficeShortName, r.Surname, r.CustomerName, r.Otchestvo, r.IdentificationNumber, r.CreditPurpose, r.ProductPrice, r.AmountDownPayment, r.RequestSumm, r.AgentUsername, r.RequestDate, r.RequestStatus, r.GroupID, r.OrgID, r.StatusDate }).ToList();
                var lst6 = (from r in lst5
                            join g in dbRWZ.Groups on r.GroupID equals g.GroupID
                            select new { r.RequestID, r.CreditID, r.ShortName, r.OfficeShortName, r.Surname, r.CustomerName, r.Otchestvo, r.IdentificationNumber, r.CreditPurpose, r.ProductPrice, r.AmountDownPayment, r.RequestSumm, r.AgentUsername, r.RequestDate, r.RequestStatus, g.GroupName, r.OrgID, r.StatusDate }).ToList();
                var lst7 = (from r in lst6
                            join o in dbRWZ.Organizations on r.OrgID equals o.OrgID
                            select new { r.RequestID, r.CreditID, r.ShortName, r.OfficeShortName, r.Surname, r.CustomerName, r.Otchestvo, r.IdentificationNumber, r.CreditPurpose, r.ProductPrice, r.AmountDownPayment, r.RequestSumm, r.AgentUsername, r.RequestDate, r.RequestStatus, r.GroupName, o.OrgName, r.StatusDate }).ToList();
        ...
        protected void btnSaveCustomer_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                //DateTime DocumentValidTill = Convert.ToDateTime(tbValidTill.Text); //DateTime.ParseExact(ValidTill, "dd.MM.yyyy", null);
                ...
                if (DateOfBirth != DateOfBirthINN) {
                    ...
                    else { MsgBox("Не полностью заполнены паспортные данные клиента в базе Банка, обратитесь к кредитному специалисту", this.Page, this); f = false; }
                }
                ...
                if (f)
                {
                    if (hfCustomerID.Value == "noselect")
                    {
                        ...
                        if (cust.Count == 0)
                        {
                            ...
                            {
                                dCustomer.Data = new GeneralController.Data();
                    ...
                    else
                    {
                        ...
                        //else { CustID = Convert.ToInt32(hfGuarantorID.Value); }
                        ...
                        //cust.IssueDate = Convert.ToDateTime(tbIssueDate.Text);
                        ...
                        //cust.DocumentValidTill = Convert.ToDateTime(tbValidTill.Text);
                        ...
                        //cust.DateOfBirth = Convert.ToDateTime(tbDateOfBirth.Text);
        ...
        protected void btnSendCreditRequest_Click(object sender, EventArgs e)
        {
...

            {
                if (hfRequestAction.Value == "new")
                {
                    ...
                    var cardN = dbRWZ.CardRequests.Where(r => ((r.CardN == tbCardN.Text) && ((r.RequestStatus != "Отменено") || ((r.RequestStatus != "Отменено"))))).ToList();
                    if (cardN.Count > 0)
                    {
                        MsgBox("Проверьте номер карты, карта с таким номером используется", this.Page, this);
                        gvAllReqCust.DataSource = cardN;
                    ...
                    else
                    {
                        ...
                        {
                            if (hfCustomerID.Value == "noselect") { MsgBox("Перед сохранением заявки нужно выбрать клиента", this.Page, this); lblMessageClient.Text = "Перед сохранением заявки нужно выбрать клиента"; }
                            if (!Page.IsValid) { MsgBox("Нужно заполнить все поля заявки", this.Page, this); }
                        }
                    }
                }
                else
        ...
        //    string templateAdm = "";
        //    string templateUsr = "";
        //    dbdataDataContext db = new dbdataDataContext(connectionStringR);
        //    dbdataDataContext dbRWZ = new dbdataDataContext(connectionStringRWZ);
        //    int usrID = Convert.ToInt32(Session["UserID"].ToString());
        ...
        //    int amoundp = 10;
        //    dbdataDataContext db = new dbdataDataContext(connectionString);
        //    var lstRequestsProducts = db.RequestsProducts.Where(r => r.RequestID == Convert.ToInt32(hfRequestID.Value)).ToList();
        ...
        public void editcommand(int id)
        {
            ...
            try
            {
                ...
                if (Convert.ToInt32(Session["RoleID"]) == 20) //Эксперты ГБ Капитал
                {
                    lblCodeWord.Visible = true; tbCodeName2.Visible = true;
                }
                ...
                btnProffer.Visible = true; //предложение зп
                ...
                if (lst.RequestStatus == "Новая заявка")
                {
                    ...
                    if (Convert.ToInt32(Session["RoleID"]) == 23) //Агенты Карты
                    {
                        btnCancelReq.Visible = true;  //Отменено
                        //btnFixed.Visible = true; //Исправлено
                        btnConfirm.Visible = true; //Подтверждено
                    ...
                    if ((Convert.ToInt32(Session["RoleID"]) == 4) || (Convert.ToInt32(Session["RoleID"]) == 9) || (Convert.ToInt32(Session["RoleID"]) == 23)) //Админы Билайн
                    {
                        btnCancelReq.Visible = true; //Отменено
                ...
                //    if (Convert.ToInt32(Session["RoleID"]) == 19) //Эксперты Капитал
                //    {
        ...
        protected void DeleteFile(object sender, EventArgs e)
        {
            ...
            dbdataDataContext dbRWZ = new dbdataDataContext(connectionStringRWZ);
            dbRWZ.CardRequestsFiles.DeleteAllOnSubmit(from v in dbRWZ.CardRequestsFiles where (v.ID == id) select v);
        ...
        //if (Convert.ToInt32(Session["RoleID"]) == 1))   //Агенты Билайн и Нур
                                ...
                                {
                                    {
                                        CardRequestsFile newRequestFile = new CardRequestsFile
                                        {
                                            ...
                                            FullName = "\\Cards\\" + partnerdir + "\\" + dateRandodir + "\\" + destinationFile,
                                            FullName2 = fileupl + "\\Cards\\" + partnerdir + "\\" + dateRandodir + "\\" + destinationFile,
                                            FileDescription = tbFileDescription.Text,
                                            //IsPhoto = false
                                        };
                                        ItemController ctl = new ItemController();
        ...
        {
            string destinationFolder = Server.MapPath("~/") + "Cards\\" + partnerdir; // 1-вариант
        ...
        protected string UploadImageAndSave(bool hasfile, string filename) //main function
        {
            ...
            if (hasfile)
            {
                ...
                while (System.IO.File.Exists(filepath))
                {
                    ...
                    filepath = Server.MapPath("~/") + "\\Cards\\" + partnerdir + "\\" + filename;
                }
        ...
        private void Activated()
        {
            dbdataDataContext dbRWZ = new dbdataDataContext(connectionStringRWZ);
            DateTime dateTimeNow = dateNow;
            var lstCard = (from v in dbRWZ.CardRequests where (v.RequestID == Convert.ToInt32(hfRequestID.Value)) select v).ToList().FirstOrDefault();
            if (lstCard != null)
            {
                lstCard.RequestStatus = "Активировано";
                lstCard.StatusDate = dateTimeNow;
                dbRWZ.CardRequests.Context.SubmitChanges();
                refreshGrid();
                string hexGreen = "#7cfa84";
                Color _colorGreen = System.Drawing.ColorTranslator.FromHtml(hexGreen);
                ...
                CreditController ctx = new CreditController();
                ctx.ItemRequestHistoriesAddItem(newItem);
                string usernameAgent = lstCard.AgentUsername;
                string fullnameAgent = lstCard.AgentFirstName;
                string fullnameCustomer = lstCard.Surname + " " + lstCard.CustomerName + " " + lstCard.Otchestvo;
                int usrID = Convert.ToInt32(Session["UserID"].ToString());
                int reqID = lstCard.RequestID;

                if (lstCard.CreditRequestID != null)
                {
                    var lst = dbRWZ.Requests.Where(x => x.RequestID == lstCard.CreditRequestID).FirstOrDefault();
                    lst.RequestStatus = "К подписи";
                    lst.CardAccount = lstCard.AccountN;
        ...
        private void Signature()
        {
            ...
            if (lst5 != null)
            {
                ...
                CreditController ctx = new CreditController();
                ctx.ItemRequestHistoriesAddItem(newItem);
                //var Customer = db.CardRequests.Where(r => r.RequestID == Convert.ToInt32(hfRequestID.Value)).FirstOrDefault();
        ...
        //        if (boolagent) //отправка сообщ агенту
        //        {
        //            int? agentID = dbRWZ.CardRequests.Where(r => r.RequestID == reqID).FirstOrDefault().AgentID;
        ...
        private void ExportGridToExcel()
        {
            ...
            gvRequests.GridLines = GridLines.Both;
            gvRequests.HeaderStyle.Font.Bold = true;
            gvRequests.RenderControl(htmltextwrtter);
