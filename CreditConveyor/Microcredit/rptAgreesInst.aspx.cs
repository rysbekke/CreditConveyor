            ...
            //try 
            {
                ...
                if (lstGraphics.Count > 0)
                {
                    ...
                    if (orgID == 7) // Только для Мой Телефон
                    {
                        ...
                        string shopName = (shopID != null) ? dbRWZ.Shops.Where(r => r.ShopID == shopID).FirstOrDefault().ShopName : "Мой Телефон";
                        string shopNameAddress = (shopID != null) ? dbRWZ.Shops.Where(r => r.ShopID == shopID).FirstOrDefault().ShopAddress : "";
