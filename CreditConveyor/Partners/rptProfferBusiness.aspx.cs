...
namespace СreditСonveyor.Partners
{
    public partial class rptProfferBusiness : System.Web.UI.Page
    {
        ...
        static string connectionStringRWZ = ConfigurationManager.ConnectionStrings["connectionStringZ"].ToString(); dbdataDataContext dbRWZ = new dbdataDataContext(connectionStringRWZ);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!zarplat)
            {
                ...
                if (chp > 0)
                {
                    ...
                    y1 = 100 * cho / chp;
                    if ((cho < 0) && (chp < 0)) y1 = y1 * (-1);
                    y2 = 100 * (k + OtherLoans) / (valp + Convert.ToDouble(request.AdditionalIncome));
                    if ((y1 >= 20) && (y2 < y22)) f = true;
